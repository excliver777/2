...
exports.quiz = function (message, time, embed_color="0000ff") {
    ...
    message.channel.send (new Discord.MessageEmbed()
        .setAuthor(`질문: ${q.question}`, message.client.user.avatarURL)
        .setDescription(
            `A ) ${a[0]}
            B ) ${a[1]}
            C ) ${a[2]}
        ...
        msg.awaitReactions(filter, { max: 1, time: time*1000, errors: ['time'] })
            .then(collected => {
                ...
                if (reaction == '🇦') {
                    if (a[0] == q.answer) message.channel.send(new Discord.MessageEmbed ().setDescription(`${challanger} 어케 알았노`).setColor('00ff00'))
                    else message.channel.send (new Discord.MessageEmbed ().setDescription(`${challanger} 틀렸어요.. ㅎ`).setColor("ff0033"));
                }
                else if (reaction == '🇧') {
                    if (a[1] == q.answer) message.channel.send(new Discord.MessageEmbed ().setDescription(`${challanger} 어케 알았노`).setColor('00ff00'))
                    else message.channel.send (new Discord.MessageEmbed ().setDescription(`${challanger} 틀렸어요.. ㅎ`).setColor("ff0033"));
                }
                else if (reaction == '🇨') {
                    if (a[2] == q.answer) message.channel.send(new Discord.MessageEmbed ().setDescription(`${challanger} 어케 알았노`).setColor('00ff00'))
                    else message.channel.send (new Discord.MessageEmbed ().setDescription(`${challanger} 틀렸어요.. ㅎ`).setColor("ff0033"));
                }
                else if (reaction == 'd') {
                    if (a[3] == q.answer) message.channel.send(new Discord.MessageEmbed ().setDescription(`${challanger} 어케 알았노`).setColor('00ff00'))
                    else message.channel.send (new Discord.MessageEmbed ().setDescription(`${challanger} 틀렸어요.. ㅎ`).setColor("ff0033"));
                }
            })
            .catch (error => {
                message.channel.send(new Discord.MessageEmbed().setDescription("지구가 멸망할때까지 고민하실 건가요? 시간초과입니다.").setColor("ff0033"));
            });
    })
}